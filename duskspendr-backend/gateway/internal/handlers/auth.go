...
func (h *AuthHandler) Start(w http.ResponseWriter, r *http.Request) {
	...
	if !utils.ValidatePhoneE164(input.Phone) {
  ...
  `, now, input.Phone)
	...
	codeHash := utils.HashOTP(h.Config.AuthPepper, otpID, code)
...
func (h *AuthHandler) Verify(w http.ResponseWriter, r *http.Request) {
	...
	if !utils.ValidatePhoneE164(input.Phone) || input.Code == "" {
  ...
  `, input.Phone).Scan(&otpID, &userID, &expiresAt, &codeHash, &attemptsRemaining)
	...
	expectedHash := utils.HashOTP(h.Config.AuthPepper, otpID, input.Code)
	if !utils.SafeCompare(codeHash, expectedHash) {
		_, _ = h.Pool.Exec(r.Context(), `
      UPDATE auth_otps
